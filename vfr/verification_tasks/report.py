...
def get_data(env, vfdb, gd, grid_state, opts, fpu_id, fpu_config):
    ...
    beta_collision_result = get_angular_limit(
        env, vfdb, fpu_id, serialnumber, "beta_collision"
    )

    datum_repeatability_result = get_datum_repeatability_result(
        env, vfdb, fpu_id, serialnumber
    )
    ...
    metrology_height_result = get_metrology_height_result(
        env, vfdb, opts, fpu_config, fpu_id
    )
    metrology_height_images = get_metrology_height_images(
        env, vfdb, opts, fpu_config, fpu_id
    )
    ...
    pupil_alignment_result = get_pupil_alignment_result(
        env, vfdb, opts, fpu_config, fpu_id
    )
    pupil_alignment_images = get_pupil_alignment_images(
        env, vfdb, opts, fpu_config, fpu_id
    )
