...
MOONS FPS Simulator Plotting Module.
...
10 Oct 2013: Created (from the MIRI plotting module of the same name)
...
18 Jan 2016: Corrected inefficiency in circle plotting. Added an ellipse plot.
17 Aug 2016: Corrected bug in ellipse plotting.
23 Aug 2016: Added missing Python 3 emulation.
07 Oct 2016: Added equal_aspect parameter. Make circle plotting more efficient.
05 Jun 2017: Added ylevels array to the plot_xycolumn function.
...
logger = logging.getLogger("mocpath.plotting")
...
def plot_circles(xdata, ydata, radii, pyplt=plt, plotfig=None, plotaxis=None,
            figsize=(10,6), equal_aspect=False, showplot=None,
            xscale='linear', yscale='linear', facecolor='w', circcolor='y ',
            linefmt='b-', linestyle='-', xlabel='', ylabel='', title='',
            grid=True, filled=False, **kwargs):
    """
...
def plot_ellipses(xdata, ydata, majors, minors, tilts, pyplt=plt, plotfig=None,
            plotaxis=None, figsize=(10,6), equal_aspect=False, showplot=None,
            xscale='linear', yscale='linear', facecolor='w', ellipsecolor='y ',
            linefmt='b-', linestyle='-', xlabel='', ylabel='', title='',
            grid=True, **kwargs):
    """
...
def plot_xycolumn(xdata, ylist, ylevels=None, yerrlist=None, pyplt=plt, figsize=(8,10),
    ...
    ylevels: list of float
        List of Y levels to show on each plot.
    ...
    def _doplot(subno, plotno, subcols, thisplt, thisfig, xscale, yscale,
                xlabel, ylabels, plotxdata, ydata, ylevel, yerr, **kwargs):
        ...
        if ylabels is not None and len(ylabels)>plotno:
            ...
            ystrg = ''
        plotaxis = plot_xy(plotxdata, plotydata, yerr=yerr,
        ...
        if ylevel is not None:
            ylevdata = np.ones_like(plotxdata) * ylevel
            plot_xy(plotxdata, ylevdata, plotaxis=plotaxis, linefmt='r-',
    ...
    if subcols > maxplots:
        subcols = maxplots
    if ylevels is None and yerrlist is None:
        for ydata in ylist:
            ...
            _doplot(subno, plotno, subcols, plt, fig, xscale, yscale, xlabel,
                    ylabels, plotxdata, ydata, None, None, **kwargs)
            subno += 1
            plotno += 1
    elif yerrlist is None:
        for ydata,ylevel in zip(ylist,ylevels):
            (page, subno, fig) = _dopage(pyplt, page, subno, fig, maxplots,
                                         maxfigures, figsize, equal_aspect, title)
            if page > maxfigures:
                break
            _doplot(subno, plotno, subcols, plt, fig, xscale, yscale, xlabel,
                    ylabels, plotxdata, ydata, ylevel, None, **kwargs)
            subno += 1
            plotno += 1
    elif ylevels is None:
        for ydata,yerr in zip(ylist,yerrlist):
            ...
            if page > maxfigures:
                break
            _doplot(subno, plotno, subcols, plt, fig, xscale, yscale, xlabel,
                    ylabels, plotxdata, ydata, None, yerr, **kwargs)
            subno += 1
            plotno += 1
    else:
        for ydata,yerr in zip(ylist,yerrlist):
...
def plot_image3D(data, pyplt=plt, plotfig=None, figsize=(10,10),
                 equal_aspect=False, showplot=True, datatype='matrix',
                 datascale='linear', maxplots=16, maxfigures=8, cmap='hot',
                 withbar=False, clip=None, xlabel='', ylabel='',
                 zlabel='Slice', title='', **kwargs):
    """
...
def plot( *args ):
    ...
    plt: matplotlib pyplot object
        The top level pyplot object, imported by the plotting module.
