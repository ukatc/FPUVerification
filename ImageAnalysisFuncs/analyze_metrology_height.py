...
def methtHeight(image_path,	#configurable parameters
	...
	if betaSide == 0:
		raise MetrologyHeightAnalysisError("Beta arm side not found - consider changing scan height")
	...
	if stdSmallTarget > METHT_STANDARD_DEV:
		raise MetrologyHeightAnalysisError("Small target points have high standard deviation"
                                                   " - target may not be sitting flat")
        
	if stdLargeTarget > METHT_STANDARD_DEV:
		raise MetrologyHeightAnalysisError("Large target points have high standard deviation"
                                                   " - target may not be sitting flat")
        
	if noiseMetric > METHT_NOISE_METRIC:
		raise MetrologyHeightAnalysisError("Image noise excessive - consider "
                                                   "changing Gaussian blur value")
